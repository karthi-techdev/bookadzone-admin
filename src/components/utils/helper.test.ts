...
jest.mock('country-state-city', () => ({
  Country: { getAllCountries: jest.fn(() => [{ name: 'India', isoCode: 'IN' }]) },
  State: { getStatesOfCountry: jest.fn(() => [{ name: 'Tamil Nadu', isoCode: 'TN' }]) },
  City: { getCitiesOfState: jest.fn(() => [{ name: 'Chennai' }]) }
}));
