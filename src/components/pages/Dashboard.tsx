...
import {
  FiMail, FiPhone, FiGlobe, FiCalendar, FiCheckCircle, FiClock,
  FiMapPin, FiDollarSign, FiPieChart, FiTrendingUp, FiUser, FiHome,
...
type NotificationType = 'booking' | 'payment' | 'campaign';
...
const Dashboard: React.FC = () => {
  const [profileCompletion] = useState<number>(75);
  const [kycStatus] = useState<'Approved' | 'Pending' | 'Rejected'>('Approved');

  // Chart config types may require installing @types/apexcharts
  ...
  const campaignSeries: number[] = [12, 8, 15, 3];
  ...
  const revenueOptions: ApexCharts.ApexOptions = {
    ...
    xaxis: {
      ...
      labels: {
        style: { fontSize: '10px', colors: '#98A9B8' },
      },
    },
    yaxis: {
      labels: {
  ...
  const slotOptions: ApexCharts.ApexOptions = {
    chart: {
      type: 'donut',
      height: 300,
      fontFamily: 'Lexend Variable, sans-serif',
