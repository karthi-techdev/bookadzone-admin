...
const CampaignDetails = () => {
  const [status, setStatus] = useState('Approved (Waiting for payment)');
  const [rejectionReason, setRejectionReason] = useState('');
  const [showRejectionForm, setShowRejectionForm] = useState(false);
  const [showAgreementPaymentModal, setShowAgreementPaymentModal] = useState(false);
  const [installationStatus, setInstallationStatus] = useState('not_started');
  const [campaignPhoto, setCampaignPhoto] = useState(null);
  const [selectedAgreement, setSelectedAgreement] = useState('');
  const [agreementStep, setAgreementStep] = useState('agreement'); // 'agreement' or 'payment'
  ...
  const agreementPaymentModalRef = useRef(null);
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      // Simulate sending photo to advertiser
      setTimeout(() => {
  ...
  const calculatePaymentTotal = () => {
    ...
    const subtotal = parseFloat(mediaRental) + 
                    parseFloat(printingCost) + 
                    parseFloat(materialCost) + 
                    parseFloat(designCost) + 
                    parseFloat(installationCost) + 
                    parseFloat(complianceFee) + 
                    parseFloat(convenienceFee);
    
    const taxAmount = subtotal * (parseFloat(gst) / 100);
  ...
  const handlePrintingTypeChange = (e) => {
  ...
  const handleSendAgreementWithPayment = () => {
    if (agreementStep === 'agreement') {
      ...
      // Here you would typically generate and send the agreement with payment link
  ...
  const handleInstallationStatusChange = (newStatus) => {
  ...
  // Handle outside click for modals - fixed version
  useEffect(() => {
    const handleOutsideClick = (e) => {
      if (showAgreementPaymentModal && 
          agreementPaymentModalRef.current && 
          !agreementPaymentModalRef.current.contains(e.target)) {
        setShowAgreementPaymentModal(false);
        setAgreementStep('agreement');
      }
    };
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {installationStatus === 'in_progress' && (
                  <button 
                    ...
                    disabled={!campaignPhoto && installationStatus === 'completed'}
                  >
