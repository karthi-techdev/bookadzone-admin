...
const AddProperties = () => {
  ...
  const completionPercentage = Math.floor((step / steps.length) * 100);
  ...
  const removeImage = (index) => {
    const newImages = [...formData.propertyImages];
  ...
  // Options for selects
  const categories = ['Billboard', 'Hoarding', 'Unipole', 'Digital Billboard', 'Banner'];
  const locationTypes = ['Urban', 'Highway', 'Commercial', 'Others'];
  const orientations = ['Portrait', 'Landscape'];
  const pricingTypes = ['Per Day', 'Per Week', 'Per Month'];
  const zoneTypes = ['Residential', 'Commercial', 'Mixed'];
  const interestOptions = ['Shopping', 'Tech', 'Fashion', 'Education', 'Finance', 'Entertainment', 'Food', 'Travel'];
  const fileTypeOptions = ['JPG', 'PNG', 'GIF', 'PDF', 'PSD', 'AI'];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-[var(--dark-color)] border border-[var(--light-blur-grey-color)] rounded text-xs text-white focus:outline-none focus:ring-1 focus:ring-[var(--puprle-color--puprle-color)] h-32"
                >
